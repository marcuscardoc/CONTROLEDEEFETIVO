...
//-----------------------------------------------------------METODOS--------------------------------------------------------------------       
        ...
        private void initialize() {
        	...
        	tcSaramTelaEfetivo.setCellValueFactory(cellData -> new SimpleIntegerProperty(cellData.getValue().getSaram()).asObject());
        	...
        	tcTurmaTelaEfetivo.setCellValueFactory(cellData -> new SimpleDoubleProperty(cellData.getValue().getTurma()).asObject());
        	...
        	tcPostoDeSvTelaEfetivo.setCellValueFactory(cellData -> new SimpleStringProperty(cellData.getValue().getPostoDeSv())); 
    	    ...
    	    tcCpfTelaEfetivo.setOnEditCommit( event -> {        	       	        	 
             ...
             // Verifica se está no formato correto usando regex
       	    ...
       	    tcSenhaPortalTelaEfetivo.setCellValueFactory(cellData -> new SimpleStringProperty(cellData.getValue().getSenhaPortal()));
        	...
        	tcSaramControleMensal.setCellValueFactory(cellData ->  new SimpleIntegerProperty(cellData.getValue().getSaram()).asObject());     	
        	tcTurmaControleMensal.setCellValueFactory(cellData -> new SimpleDoubleProperty(cellData.getValue().getTurma()).asObject());
        	tcPostoDeSvControleMensal.setCellValueFactory(cellData -> new SimpleStringProperty(cellData.getValue().getPostoDeSv()));                    	
            ...
            //TABELA FERIAS
             ...
             tcTurmaFerias.setCellValueFactory(cellData -> new SimpleDoubleProperty(cellData.getValue().getTurma()).asObject());
             tcPostoDeSvFerias.setCellValueFactory(cellData -> new SimpleStringProperty( cellData.getValue().getPostoDeSv()));
             
            
             tcPeriodoAquisitivoFerias.setCellValueFactory(cellData -> new SimpleIntegerProperty(cellData.getValue().getPeriodoAquisitivo()).asObject());
             ...
             tcDataInicioFerias.setCellValueFactory(cellData -> new SimpleStringProperty( cellData.getValue().getdataInicio()));
             ...
             tcDataTerminoFerias.setCellValueFactory(cellData -> new SimpleStringProperty( cellData.getValue().getdataTermino()));
     	    ...
     	    TCQuantidadeDispMedica.setCellValueFactory(cellData -> new SimpleIntegerProperty(cellData.getValue().getQuantidade()).asObject());
     	    ...
     	    TCApartirDeDispMedica.setCellValueFactory(cellData -> new SimpleStringProperty(cellData.getValue().getApartirDe()));
     	   ...
     	   TCTipoDeDispensa.setCellValueFactory(cellData -> new SimpleStringProperty(cellData.getValue().getTipoDeDispensa()));
     	 ...
     	 TCMedicoDispMedica.setCellValueFactory(cellData -> new SimpleStringProperty(cellData.getValue().getNomeMedico()));
        ...
        private void carregarObservacoesPorData() {
            ...
            try {
                for (Soldado soldado : soldadosData) {
                    ...
                    if (observacao != null) {
                        ...
                        soldado.setObservacaoTemp(""); // Define como vazio se não houver observação para a data
                    }
                }
            } catch (Exception e) {
                e.printStackTrace();
            ...
            // Atualiza a TableView após carregar as observações
            tControleMensal.refresh();
        }    
       ...
       public Observacao buscarObservacaoPorSoldadoEData(Soldado soldado, LocalDate data) {
           ...
           try {
        	   ...
        	   TypedQuery<Observacao> query = em.createQuery("SELECT o FROM Observacao o WHERE o.soldado = :soldado AND o.data = :data", Observacao.class);
      ...
      //DATEPICKER
        ...
        private void manipulandoDatePicker() {
        	    LocalDate selectedDate = dpDateControleMensal.getValue();
